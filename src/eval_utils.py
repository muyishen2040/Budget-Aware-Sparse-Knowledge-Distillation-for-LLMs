...
def calculate_budget(method: str, **kwargs) -> int:
    ...
    if method == "full":
        ...
        # avg_k is the mean valid_k across the dataset; fall back to k_max=16
        avg_k = kwargs.get("avg_k", 16.0)
        # ids + probs per token on average
        return int(round(avg_k * 2))
    else:
...
def extract_qualitative_masks(
    ...
    log_probs = F.log_softmax(teacher_logits.detach(), dim=-1)
    ...
    # Relaxed condition to capture more "competitive" cases
    ambiguous_mask = (prob_diff <= ambiguity_threshold) & (top2_probs[..., 0] > 0.05)
    ...
    # Ignore padded labels/ignored indices
