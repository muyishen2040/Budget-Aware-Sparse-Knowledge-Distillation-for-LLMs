...
from src.losses import compute_cached_adaptive_topk_tail_summary_kd_loss
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="output/adaptive_topk_tail_summary")
    parser.add_argument(
    ...
    parser.add_argument("--tail_weight", type=float, default=0.1,
        help="Beta coefficient for the tail summary KL term. 0.0 = pure adaptive top-K, 1.0 = equal weight.")
    ...
    print(f"Starting Adaptive Top-K Tail Summary KD training on device: {device} | Detected GPU: {gpu_name}")
    student.train()
    ...
    for epoch in range(num_epochs):
        for batch in tqdm(train_loader):
            ...
            # USE NEW LOSS FUNCTION HERE:
            loss, ce_loss, kl_loss = compute_cached_adaptive_topk_tail_summary_kd_loss(
